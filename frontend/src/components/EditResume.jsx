...
//resize obsever hook
const LOCAL_STORAGE_KEY = "resume_cache";
...
//fetching resume details by id or backend url
  ...
  try {
    ...
    if (response.data && response.data.profileInfo) {
      ...
      setResumeData({
        ...
        template: resumeInfo.template || { theme: "modern", colorPalette: [] },
        profileInfo: resumeInfo.profileInfo || { fullName: "", designation: "", summary: "" },
        ...
        skills: resumeInfo.skills || [{ name: "", progress: 0 }],
        ...
        languages: resumeInfo.languages || [{ name: "", progress: 0 }],
...
//it will help in choosing the preview and downloading the resume and save resume as an image
const uploadResumeImages = async () => {
    try {
      setIsLoading(true)

      const thumbnailElement = thumbnailRef.current
      console.log("Uploading images for ID:", resumeId);
      if (!thumbnailElement) {
        throw new Error("Thumbnail element not found")
      }
       console.log("Creating thumbnail canvas...");
      const fixedThumbnail = fixTailwindColors(thumbnailElement)
      console.log("Thumbnail element:", thumbnailElement)
console.log("Bounding box:", thumbnailElement.getBoundingClientRect())
...
await new Promise(resolve => setTimeout(resolve, 500)) 
      ...
      console.log("Canvas size:", thumbnailCanvas.width, "x", thumbnailCanvas.height);
console.log("Data URL length:", thumbnailDataUrl.length);
console.log("File size:", thumbnailFile.size);
      console.log("Thumbnail dataUrl length:", thumbnailDataUrl.length);
console.log("Thumbnail file size before upload:", thumbnailFile.size);
    ...
    const cachedResumes = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY)) || {};
    cachedResumes[resumeId] = resumeData;
    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(cachedResumes));
    console.log("Saved resume progress to localStorage:", resumeId);
...
//theme selector function
 ...
 useEffect(() => {
  ...
  const cachedResumes = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY)) || {};
  const cachedResume = cachedResumes[resumeId];

  if (cachedResume) {
    console.log("Loaded resume from localStorage:", resumeId);
  ...
  return (
    ...
    top: "-9999px",
    left: "-9999px",
    width: "800px", // or whatever size your resume needs
    background: "white"
  }} ref={thumbnailRef}>
