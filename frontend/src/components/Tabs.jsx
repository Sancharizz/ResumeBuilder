...
const Tabs = ({tabs, activeTab, setActiveTab}) => {
  return (
    <div className='w-full my-2'>
        <div className=' flex felx-wrap bg-violet-50 p-1 rounded-2xl border-violet-100'>
            {
                tabs.map((tab) =>(
                    ...
                    className={`relative flex-1 sm:flex-none px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm
                        ...
                        onClick={() => setActiveTab(tab.label)}>
                            <span className=' relative z-10'>
                                {tab.label}
                                {activeTab === tab.label && (
                                    <div className=' absolute inset-0 bg-linear-to-r from-violet-500/10 to-fuchsia-500/10 rounded-xl'></div>
                                )}
                            </span>
                        </button>
                ))
